...
namespace Gyvr.Mythril2D
{
    ...
    public class UIEventLog : MonoBehaviour
    {
        ...
        private bool ShouldLog(EItemTransferType transferType) => m_itemTransferTypesToLog.Contains(transferType);

        private void OnItemAdded(Item item, int count, EItemTransferType transferType)
        {
            if (ShouldLog(transferType))
            {
                Log(m_itemAdded, item.displayName, count);
            }
        }

        private void OnItemRemoved(Item item, int count, EItemTransferType transferType)
        {
            if (ShouldLog(transferType))
            {
                Log(m_itemRemoved, item.displayName, count);
            }
        }
