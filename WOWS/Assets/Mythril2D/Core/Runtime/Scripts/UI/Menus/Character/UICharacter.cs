...
{
    public class UICharacter : AUIMenu
    {
        [Header("References")]
        ...
        {
            return m_stats.Length > 0 ? m_stats[0].GetFirstButton().gameObject : base.FindSomethingToSelect();
        }
        ...
        private void UpdateStatsSection()
        {
            foreach (UICharacterStat stat in m_stats)
            {
                stat.UpdateUI(m_character.stats.values, m_tempStats);
            }
        }
