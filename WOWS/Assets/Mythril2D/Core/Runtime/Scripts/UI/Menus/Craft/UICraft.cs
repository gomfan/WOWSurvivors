...
namespace Gyvr.Mythril2D
{
    public class UICraft : AUIMenu
    {
        ...
        private async void OnRecipeEntryClicked(Recipe recipe)
        {
            if (m_craftingStation.CanCraft(recipe, out bool hasMoney, out bool hasIngredients))
            {
                await GameManager.DialogueSystem.Main.PlayNow(DialogueUtils.CreateDialogueTree(m_craftSucceeded, null, recipe.displayName));
            ...
            {
                await GameManager.DialogueSystem.Main.PlayNow(DialogueUtils.CreateDialogueTree(m_cannotCraftBecauseIngredients, null, recipe.displayName));
            }
            else if (!hasMoney)
            {
                await GameManager.DialogueSystem.Main.PlayNow(DialogueUtils.CreateDialogueTree(m_cannotCraftBecauseMoney, null));
            }
        }

        private async void OnBagItemClicked(Item item)
        {
            await GameManager.DialogueSystem.Main.PlayNow(DialogueUtils.CreateDialogueTree(m_cannotUseItem, null, item.displayName));
        }
