...
namespace Gyvr.Mythril2D
{
    public enum EMessagePropagationMode
    {
        ...
        SendMessage,
        SendMessageUpwards,
        SendMessageToAnimationMessageReceiver
    }
    ...
    public class StateMessageDispatcher : StateMachineBehaviour
    {
        ...
        private void PropagateMessage(Component source, MessageData messageData)
        {
            switch (messageData.propagationMode)
            {
                ...
                case EMessagePropagationMode.SendMessageToAnimationMessageReceiver:
                    source.GetComponentInParent<IAnimationMessageReceiver>()?.DispatchAnimationMessage(messageData.message, messageData.options);
                    break;
            }
        }
    }
